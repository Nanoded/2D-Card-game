...
Поскольку в ТЗ не была прописана система заполнения колоды, колода заполняется случайными картами из заранее подготовленного набора. В референсе присутствует сохранение ОЗ игрока между уровнями(комнатами), но по тз подобного не было и потому не реализовано. Реализация подобного делается при помощи скриптовых объектов. Поскольку в проекте уже есть скриптовый объект, то я думаю не возникнет сомнений в том, что я умею ими пользоваться.
